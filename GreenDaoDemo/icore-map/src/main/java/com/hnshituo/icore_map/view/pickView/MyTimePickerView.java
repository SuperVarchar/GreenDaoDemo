...
public class MyTimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    public MyTimePickerView(Builder builder) {
        ...
        this.isHalfHour = builder.isHalfHour;
        this.isShowWeek = builder.isShowWeek;
    ...
    public static class Builder {
        private int layoutRes = R.layout.custom_pickerview_time;
        private CustomListener customListener;
        ...
        public Builder setHalfHour(boolean HalfHour) {
            this.isHalfHour = HalfHour;
            return this;
        }
        public Builder setIsShowWeek(boolean isShowWeek) {
    ...
    private void initView(Context context) {
        ...
        myWheelTime = new MyWheelTime(timePickerView,type, gravity, Size_Content);
        myWheelTime.setLunarCalendar(isLunarCalendar);
        ...
        setTime();
        myWheelTime.setLabels(label_year, label_month, label_day, label_hours, label_mins, label_seconds);
        myWheelTime.setTextXOffset(xoffset_year, xoffset_month, xoffset_day, xoffset_hours, xoffset_mins, xoffset_seconds);

        setOutSideCancelable(cancelable);
        myWheelTime.setCyclic(cyclic);
        myWheelTime.setDividerColor(dividerColor);
        myWheelTime.setDividerType(dividerType);
        myWheelTime.setLineSpacingMultiplier(lineSpacingMultiplier);
        myWheelTime.setTextColorOut(textColorOut);
        myWheelTime.setTextColorCenter(textColorCenter);
        myWheelTime.isCenterLabel(isCenterLabel);
    }
    ...
    private void setRange() {
        myWheelTime.setStartYear(startYear);
        myWheelTime.setEndYear(endYear);
    ...
    private void setRangDate() {
        myWheelTime.setHalfHour(isHalfHour);
        myWheelTime.setShowWeek(isShowWeek);

        myWheelTime.setRangDate(startDate, endDate);
        //如果设置了时间范围
        if (startDate != null && endDate != null) {
    ...
    private void setTime() {
        int year, month, day, hours, minute, seconds,week;
        ...
        myWheelTime.setPicker(year, month, day, hours, minute, seconds,week);
    }
    ...
    public void returnData() {
        if (timeSelectListener != null) {
            String dateStr = myWheelTime.getTime();
            // 暂时不支持秒的显示
            Date date = DateUtils.formatStringToYMDHMDate(dateStr);

            timeSelectListener.onTimeSelect(date, clickView);
//            try {
////     //           Date date = MyWheelTime.dateFormat.parse(myWheelTime.getTime());
////
////                Date date= DateUtils.formatStringToData(myWheelTime.getTime());
////
////                timeSelectListener.onTimeSelect(date, clickView);
...
//            }
    ...
    public void setLunarCalendar(boolean lunar) {
        try {
            ...
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(MyWheelTime.dateFormat.parse(myWheelTime.getTime()));
            year = calendar.get(Calendar.YEAR);
            month = calendar.get(Calendar.MONTH);
            day = calendar.get(Calendar.DAY_OF_MONTH);
            hours = calendar.get(Calendar.HOUR_OF_DAY);
            ...
            myWheelTime.setLunarCalendar(lunar);
            myWheelTime.setLabels(label_year, label_month, label_day, label_hours, label_mins, label_seconds);
            myWheelTime.setPicker(year, month, day, hours, minute, seconds,0);
        } catch (ParseException e) {
            e.printStackTrace();
        }
    }
    ...
    public boolean isLunarCalendar() {
        return myWheelTime.isLunarCalendar();
    }
