...
public class PickView {
    ...
    public interface OnSelectDataListener {
        void onSelected(BaseDataInfo info);
    }

    public static void showNotCancelPickView(BaseDataInfo currentinfo, Context context, String title, final List<BaseDataInfo> items, final OnSelectDataListener listener) {
        optionsPickerView = getPickView(currentinfo, context, title, "暂无数据", false, items, listener);
        optionsPickerView.show();
    }

    public static void showPickView(BaseDataInfo currentinfo, Context context, String title, final List<BaseDataInfo> items, final OnSelectDataListener listener) {
    ...
    public static Dialog getPickView(BaseDataInfo currentinfo,
                                     Context context, String title, String emptyString, boolean cancelVisibility, final List<BaseDataInfo> items,
        ...
        if (items.size() == 0) {
            ...
            listView.setAdapter(new QuickAdapter<BaseDataInfo>(context, R.layout.dialog_single_item, items) {
                @Override
            ...
            confirm.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    dismiss();
                    if (listener != null && selectList.size() > 0) {
